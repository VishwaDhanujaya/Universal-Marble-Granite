...
?>
<div x-data="{ mobileMenuOpen: false, scrolled: false }" x-init="$watch('mobileMenuOpen', value => { document.body.style.overflow = value ? 'hidden' : '' })" @scroll.window="scrolled = (window.pageYOffset > 20)">
    <header id="main-header" :class="{'glass shadow-sm py-4': scrolled && !mobileMenuOpen, 'bg-transparent py-6': !scrolled && !mobileMenuOpen, 'bg-brand-light shadow-none py-4': mobileMenuOpen}" class="fixed w-full top-0 z-50">
        <div class="container-custom flex justify-between items-center">
            <!-- Logo -->
            <a href="<?= $base_path ?: '/' ?>" class="flex items-center space-x-3 z-50 relative group">
                <img src="<?= $base_path ?>/assets/images/logo.png" alt="Universal Marble & Granite Logo" class="h-10 w-auto object-contain transition-transform duration-300 group-hover:scale-105">
                <span :class="{'text-brand-dark': scrolled || mobileMenuOpen, 'text-white': !scrolled && !mobileMenuOpen}" class="text-xs min-[375px]:text-sm sm:text-lg md:text-2xl font-display font-bold uppercase tracking-wide transition-colors duration-300 group-hover:opacity-80">
                    Universal <span :class="{'text-brand-accent': scrolled || mobileMenuOpen, 'text-brand-accent-light': !scrolled && !mobileMenuOpen}" class="transition-colors duration-300">Marble & Granite</span>
                </span>
            </a>
            ...
            <nav class="hidden lg:flex space-x-8 items-center">
                <?php foreach ($nav_items as $name => $url): ?>
                    ...
                    $link_url = ($url == 'index.php') ? ($base_path ?: '/') : $base_path . '/pages/' . str_replace('.php', '', $url);
                    ?>
                    <a href="<?= $link_url ?>" 
    ...
    <div x-show="mobileMenuOpen" 
         ...
         class="fixed inset-0 bg-brand-light z-40 flex flex-col justify-center items-center overflow-y-auto py-20 will-change-transform-opacity" style="display: none;">
